var rows_selected = [];
$(function () {
    $("#service_entry_images").fileinput({showCaption: false,showUpload: false});
    $('#ser_date,#close_date').datetimepicker({
        format: 'YYYY-MM-DD'
    });

    var table = $('#serviceentry').DataTable( {
        "processing": true,
        "serverSide": true,
    ...
    $('#serviceentry tbody').on("click","#btnstatus",function () {

        var data = table.row($(this).parents('tr')).data();
        StatusEdit(data[1]);
    });

    $('#serviceentry tbody').on("click","#btnentry",function () {
        var data = table.row($(this).parents('tr')).data();
        ServiceEntry(data[1]);
    });

    $('#serviceentry tbody').on("click","#btnview",function () {
        var data = table.row($(this).parents('tr')).data();
    ...
    function ServiceEntry(id) {
       $('#comp_id').val(id);
       $('#serviceentry_form_modal').modal('show');
    }
...
// Handle click on checkbox
    $('#serviceentry tbody').on('click', 'input[type="checkbox"]', function(e){
...
// Handle click on table cells with checkboxes
    $('#serviceentry').on('click', 'tbody td, thead th:first-child', function(e){
...
// Handle click on "Select all" control
    $('thead input[name="select_all"]', table.table().container()).on('click', function(e){
        if(this.checked){
            $('#serviceentry tbody input[type="checkbox"]:not(:checked)').trigger('click');
        } else {
            $('#serviceentry tbody input[type="checkbox"]:checked').trigger('click');
        }
